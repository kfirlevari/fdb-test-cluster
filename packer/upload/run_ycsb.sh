...
do
    ...
    ./bin/ycsb load fdbrecordlayer -threads 8 -P "workloads/workload$workload" -p foundationdb.clusterfile=/etc/foundationdb/fdb.cluster > ./"$logsfoldername"/recdb."w$workload.load".csv
    ./bin/ycsb run fdbrecordlayer -threads 8 -P "workloads/workload$workload" -p foundationdb.clusterfile=/etc/foundationdb/fdb.cluster  > ./"$logsfoldername"/recdb."w$workload.run".csv
